...
const InstanceEmptySubTerrainCountWrapper = (props: Props) => {
    const numInstances = props.boardHexes.length
    if (numInstances < 1) return null
    const key = 'InstanceEmptySubTerrain-' + numInstances // IMPORTANT: to include numInstances in key, otherwise gl will crash on change
    return <InstanceEmptySubTerrain key={key} {...props} />
}

const InstanceEmptySubTerrain = ({ boardHexes }: Props) => {
    ...
    return (
        ...
        >
            <cylinderGeometry args={[1, 1, HEXGRID_HEX_HEIGHT / 6, 6]} />
            <meshBasicMaterial transparent
                opacity={0.15} />
        </instancedMesh>
    )
}
export default InstanceEmptySubTerrainCountWrapper
